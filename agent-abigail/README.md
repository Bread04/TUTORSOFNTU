...
On Windows, that destination is `%USERPROFILE%\.claude\skills\agent-abigail`.

Restart Claude Code, then say **"talk to Abigail"** or run `/agent-abigail`.

> Installing from a clone made before the repo split into per-tutor folders? Delete the old `~/.claude/skills/agent-abigail` and reinstall with the two commands above — pulling into it will bury her `SKILL.md` one level deeper and Claude Code will stop finding her.
