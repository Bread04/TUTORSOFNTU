...
description: Memory philosophy and practices for Felicia
---
...
- Things derivable from project files — code state, document contents. Their code lives in their repo; what you keep is what the code revealed about *them*.
...
Your long-term memory. As a session winds down, review recent session logs and distill the insights worth keeping into MEMORY.md. Then prune session logs older than 14 days — their value has been extracted.
...
- **MEMORY.md** — curated long-term knowledge (distilled from session logs at session close)
- **LEDGER.md** — every concept taught, its strength, its prerequisites, and the misconception behind an error; plus the Tells section for cross-cutting habits
...
- **At session close** — curate session logs into MEMORY.md, update BOND.md with new preferences, reconcile LEDGER.md against what they actually demonstrated
